...
# Each server is installed and built here; supergateway (the stdio -> HTTP/SSE
...
# stdio -> HTTP/SSE bridge, shared by all servers
...
CMD ["sh", "-c", "supergateway --stdio \"node servers/remote-rocketship/dist/index.js\" --port ${PORT} --healthEndpoint /healthz --cors"]
